...
namespace ContactWebModels
{
    public class Contact
    {
     ...
     [RegularExpression("(^\\d{5}(-\\d{4})?$)|(^[ABCEGHJKLMNPRSTVXY]{1}\\d{1}[A-Z]{1} *\\d{1}[A-Z]{1}\\d{1}$)", ErrorMessage = "Zip code is invalid.")]
        ...
        [Required(ErrorMessage ="The UserId is required in order to map the contact to user correctly ")]
        ...
        public string FriendlyAddress => State is null ? "" : string.IsNullOrEmpty(StreetAddress1) ? $"{City},{State.Abbreviation},{Zip}" :
            string.IsNullOrEmpty(StreetAddress2) ?

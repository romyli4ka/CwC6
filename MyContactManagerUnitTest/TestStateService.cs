...
namespace MyContactManagerUnitTest
{
    public class TestStateService
    {
        ...
        {
            CreateMocks();
            _stateService = new StateServices(_repository.Object);
        }
        ...
        {
            var state = await _stateService.GetAsync(6546);
            state.ShouldNotBeNull();
            state.Name.ShouldBe("Alabama", StringCompareShould.IgnoreCase);
            state.Abbreviation.ShouldBe("AL", StringCompareShould.IgnoreCase);
        }
